...
describe("FractionAsset", function() {
    ...
    describe("Deployment", function() {
        ...
        it("Should revert if deployed with more less than 2", async function() {
            ...
            await expect(
                fractionAssetContract.deploy(name, 1, price)
            ).to.be.revertedWith("Not enough fracitons.");
        });
        ...
        it("Should revert if name not set", async function() {
            ...
            await expect(
                fractionAssetContract.deploy("", shares, price)
            ).to.be.revertedWith("Asset name should be set.");
        });
        it("Should revert if price is equal to 0", async function() {
            ...
            await expect(
                fractionAssetContract.deploy(name, shares, 0)
            ).to.be.revertedWith("Price should be set.");
        });
    ...
    describe("Acquire Shares", function() {
        describe("Validations", function() {
            it("Should revert with the right error if not enough shares available", async function() {
                ...
                await expect(
                    fractionAsset.connect(bob).buyFractions(101, { value: price })
                ).to.be.revertedWith("Not enough fractions available."); // only 100 available;
            });
            it("Should revert with the right error not enough ethers", async function() {
                const { fractionAsset, bob } = await loadFixture(deployFractionAsset);
                await expect(
                    fractionAsset.connect(bob).buyFractions(100, { value: 1 })
    ...
    describe("Buy fractions", function() {
        it("Should emit an event on buying fractions", async function() {
            ...
            await expect(
                fractionAsset.connect(bob).buyFractions(shares, { value: price })
            ).to.emit(fractionAsset, "BuyFractions");
        });
        it("Should update the buyers amount of shares", async function() {
            const { fractionAsset, bob } = await loadFixture(BuyFractionAsset);
